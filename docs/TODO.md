...
| Rename in every view | An editor over the name itself, in list, icon and column view |
...
| Clean This Folder | `⌃⌘L` — reads one folder and suggests where things should go. Local AI is the default and recommendation: Ollama, LM Studio and llama.cpp need no key and keep the request on the Mac. Hosted APIs are only for approved data; PHI requires an executed BAA. Provider keys live in a separate mode-0600 `credentials.json`, secret-bearing files are not read, key-shaped text is removed, and the exact request is shown before it is sent. Moves are ticked row by row, followed by a result with one-step Undo |
| Global folders and folder context | Mark a folder as somewhere things may be filed from anywhere, and write a sentence about what a folder is for. A plan may not move a file anywhere else |
...
- `sftp://` opens in a built-in browser window with nothing installed, but it is not a mount: no other application can see those files. Mounting needs macFUSE and sshfs; a File Provider extension is the route that removes the dependency — issue #4. S3 is not supported at all.
