...
| Clean This Folder | `⌃⌘L` — reads one folder and suggests where things should go. The only feature that sends file contents anywhere; the key lives in the Keychain, secrets are never read or are redacted, and the exact text is shown before it is sent. Moves are ticked row by row and undo as one step |
