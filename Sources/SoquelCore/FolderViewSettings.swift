...
enum FolderViewSettings {
    ...
    /// The stored view mode, or the global one when this folder has none.
    static func viewMode(for url: URL) -> ViewMode {
        guard let stored = entry(for: url)?.viewMode, let mode = ViewMode(rawValue: stored) else {
    ...
    static func sortOrder(for url: URL) -> SortOrder {
        entry(for: url)?.sortOrder ?? Prefs.sortOrder
    }
